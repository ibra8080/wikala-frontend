...
interface SellerOption { id: number; business_name: string }
...
export default function AdminSalesPage() {
  ...
  const fetchList = useCallback(() => {
    ...
    api.get(`/finance/admin/sales/list/?${params.toString()}`)
      .then(res => setRows(res.data))
      .finally(() => setListLoading(false))
  }, [dateFrom, dateTo, seller, product, search])
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      api.get('/finance/admin/sales/list/'),
      api.get('/sellers/admin/list/'),
      api.get('/products/admin/list/'),
    ]).then(([statsRes, listRes, sellersRes, productsRes]) => {
      setStats(statsRes.data)
      setRows(listRes.data)
      setSellers(sellersRes.data.filter((s: { status: string }) => s.status === 'approved'))
      setProducts(productsRes.data)
    }).finally(() => setLoading(false))
  }, [user, _hasHydrated, router])

  const resetFilters = () => {
    setDateFrom(''); setDateTo(''); setSeller(''); setProduct(''); setSearch('')
  }
  ...
  return (
    <div>
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-[#1B2A4A]">Sales</h1>
        <p className="text-sm text-[#6B6560] mt-1">Orders & revenue analytics</p>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-[#E0DDDA] p-5 mb-4">
        <div className="grid grid-cols-2 md:grid-cols-6 gap-3">
          ...
          <div>
            <label className="text-xs text-[#6B6560] block mb-1">Seller</label>
      ...
      <div className="bg-white rounded-2xl border border-[#E0DDDA] overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {listLoading ? (
                <tr><td colSpan={7} className="text-center py-8 text-[#6B6560]">Loading…</td></tr>
              ) : rows.length === 0 ? (
                <tr><td colSpan={7} className="text-center py-8 text-[#6B6560]">No sales found</td></tr>
              ) : rows.map(r => (
                <tr key={r.id} className="border-t border-[#E0DDDA] hover:bg-[#FAFAF8]">
                  ...
                  <td className="px-4 py-3 text-[#6B6560]">{r.seller_name}</td>
